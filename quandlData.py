        ...
        if len(each_file) > 0:
            for file in each_file:
                ...
                try:
                    ...
                    one_year_later = int(unix_time + 31536009)
                    
                    try:
                        sp500_1y = datetime.datetime.fromtimestamp(one_year_later).strftime('%Y-%m-%d')
                        row = sp500_df[(sp500_1y.index == sp500_1y)]
                        sp500_1y_value = float(row["Adjusted Close"])
                    except Exception as e:
                        try:
                            sp500_1y = datetime.datetime.fromtimestamp(one_year_later-259200).strftime('%Y-%m-%d')
                            row = sp500_df[(sp500_1y.index == sp500_1y)]
                            sp500_1y_value = float(row["Adjusted Close"])
                        except Exception as e:
                            print('exception in SP 500',str(e))
                            
                    try:
                        stock_price_1y = datetime.datetime.fromtimestamp(one_year_later).strftime('%Y-%m-%d')
                        row = stock_df[(stock_df.index == stock_price_1y)]
                        stock_price_1y_value = float(row["Adjusted Close"])
                    except Exception as e:
                        try:
                            stock_price_1y = datetime.datetime.fromtimestamp(one_year_later-259200).strftime('%Y-%m-%d')
                            row = stock_df[(stock_df.index == stock_price_1y)]
                            stock_price_1y_value = float(row["Adjusted Close"])
                        except Exception as e:
                    ...
                    try:
                        stock_price = datetime.datetime.fromtimestamp(unix_time).strftime('%Y-%m-%d')
                        row = stock_df[(stock_df.index == stock_price)]
                        stock_price = float(row['Adjusted Close'])
                    except Exception as e:
                        try:
                            stock_price = datetime.datetime.fromtimestamp(unix_time-259200).strftime('%Y-%m-%d')
                            row = stock_df[(stock_df.index == stock_price)]
                            stock_price = float(row['Adjusted Close'])
                        except Exception as e:
                            print('Stock price ',str(e))

                    stock_p_change = round((((stock_price_1y_value - stock_price) / stock_price) * 100),2)
